...
- Einstellungen: Postfach · Automatisch (auto fetch at start, archive and empty the trash after 30 days) · Portale
  (switches with one sentence each, health, pages used today, the meter only from 80 % or while paused) · Dateien ·
  Sprache · Wartung · Alles zurücksetzen (a card of its own, last). First run: full page with three real,
  self-ticking steps.
...
- 35 components (Badge, BrandMark, Button primary|secondary|ghost|danger|link x sm|md|lg, Card, Chip, ChipInput,
  Count (soft|plain), Dialog, Disclosure, DragBand (macOS toolbar row), EmptyState, Field, Icon, IconTile, JobRow,
  ListRow, MenuButton (opens the OS menu), Meter, Notice, ReasonItem, ScoreRing, Segmented, SelectionBar, SettingRow,
  SideNav, Skeleton, Spinner, Splitter, StatTile, StatusLine, TextArea, TextField, Toast, Toggle, Tooltip). Not: select,
  checkbox, radio, a page-drawn context menu, window controls.
