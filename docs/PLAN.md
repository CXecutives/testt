...
| Layout | own clean modern design, no imitation of other apps (user 2026-09-24): left sidebar 232 px (brand, "Abrufen" as the window's primary, nav with icons and unread count, run status at the bottom), icon rail of 64 px below 1100 px; content gets a 40 px drag strip with the caption buttons; word tabs are gone. Test ids `nav-jobs`, `nav-profile`, `nav-settings`, `view-*` |
