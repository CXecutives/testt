...
/// Minimal template for a new profile. The keys are shared with the external matching
/// skill (external contract - do not translate); the values are empty on purpose.
...
/// Writes the empty template to `target` (a place the user chose).
