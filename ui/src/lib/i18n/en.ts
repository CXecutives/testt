...
const profileField: Record<string, string> = {
  ...
  targetYears: 'Experience of the role',
  minSalary: 'Annual salary',
  permanentPlaces: 'Places',
  permanentRemoteMin: 'Remote share',
  focus: 'Focus areas',
  roles: 'Target roles',
...
const profileKey: Record<string, string> = {
  ...
  min_jahresgehalt: 'Minimum salary',
  min_annual_salary: 'Minimum salary',
  ...
  zielprofil_min_jahre: 'Seniority',
  target_min_years: 'Seniority',
...
const warning = {
  ...
  availabilityNotUnderstood: 'The availability in the profile cannot be read.',
  ignoredKeys: (p) => {
    const keys = keyList(p.keys);
    return `${joined(keys)} ${keys.length === 1 ? 'is' : 'are'} not used.`;
  },
  criterionNotUnderstood: (p) => `The value of ${keyLabel(str(p.key))} cannot be read.`,
  regionWithoutPlaces: 'The region has no places, so the rule stays off.',
...
export const en: Catalog = {
  ...
  toolbar: {
    ...
    facetNew: 'New',
    facetAll: 'All',
    facetApplications: 'Applications',
    facetPinned: 'Saved',
    sortLabel: {
      match: 'Best match',
      newest: 'Newest',
  ...
  list: {
    ...
    emptyApplications: 'No application noted yet.',
  ...
  profile: {
    ...
    understood: (competences: number, focus: number) =>
      focus > 0
        ? `${count(competences, 'competence', 'competences')} found, ${count(focus, 'focus area', 'focus areas')}`
        : `${count(competences, 'competence', 'competences')} found`,
    warning,
    pack: {
      finance: 'Finance',
      sap: 'SAP',
