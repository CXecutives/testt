...
<Card padding="none" testid="portal-{portal.portal}">
  ...
  {#if portal.enabled || error || leftover}
    <div class="body" in:rise={{ distance: 'sm' }} out:fade>
      ...
      {#if portal.enabled}
        <div class="rows">
          <SettingRow
            ...
            hint={portal.fetchDetails ? de.settings.riskText[risk] : de.settings.detailsOff}
            for="switch-details-{portal.portal}"
            testid="details-{portal.portal}"
          >
            {#snippet badges()}
              <Badge
                label={de.settings.risk[risk]}
                tone={RISK_TONE[risk]}
                icon="shield"
                hint={de.settings.riskInfo[risk]}
              />
            {/snippet}
            <Toggle
          ...
          {#if portal.login === 'optional'}
            ...
            >
              {#snippet badges()}
                {#if risk !== 'account'}
                  <Badge
                    label={de.settings.risk.account}
                    tone="danger"
                    icon="shield"
                    hint={de.settings.riskInfo.account}
                  />
                {/if}
              {/snippet}
              <Toggle
      ...
      {#if leftover}
        <div class="rows">
          <SettingRow label={de.settings.signedIn} hint={de.settings.sessionLeft}>
            <Button
              variant="secondary"
              size="sm"
              icon="log-out"
